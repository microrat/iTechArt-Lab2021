...
const PizzaCard = props => {
  ...
  const id = idParam.id - 1;

  const addToCart = () => {
    const cartitem = { ...props.pizzas[id] };
    if (state.size === '') {
    ...
    <div className={s.card}>
      <div className={s.card__content}>
        ...
        <div className={s.card__item}>
          ...
          <form>
            <input type="radio" value="sm" id="sm" onChange={handleChange} name="size" />
            <label>Маленькая - {props.pizzas[id].size.sm.price} руб.</label>
            <input type="radio" value="md" id="md" onChange={handleChange} name="size" />
            <label>Средняя - {props.pizzas[id].size.md.price} руб.</label>
            <input type="radio" value="lg" id="lg" onChange={handleChange} name="size" />
            <label>Большая - {props.pizzas[id].size.lg.price} руб.</label>
          </form>
          <button className={s.card__button} onClick={addToCart} type="button">
            Добавить в корзину
          </button>
        </div>
      </div>
